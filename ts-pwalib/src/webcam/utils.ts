...
export function zeros(
  shape: number[],
  type: 'uint8' | 'uint16' | 'uint32' | 'int8' | 'int16' | 'int32' | 'float32' | 'float64' = 'uint8'
): TypedArray {
 ...
 */
export function indices(height: number, width: number): [Uint32Array, Uint32Array] {
 ...
 */
export function checkerboard(height: number, width: number): Uint8Array {
...
export function bitwiseOp(
  array: TypedArray,
  operation: 'and' | 'or' | 'xor' | 'not' | 'shiftLeft' | 'shiftRight',
  operand?: number
): TypedArray {
  const result = new (array.constructor as any)(array.length);
  
  switch (operation) {
    case 'and':
      if (operand === undefined) throw new Error('Operand required for bitwise AND');
      for (let i = 0; i < array.length; i++) {
        result[i] = array[i] & operand;
      }
      break;
    case 'or':
      if (operand === undefined) throw new Error('Operand required for bitwise OR');
      for (let i = 0; i < array.length; i++) {
        result[i] = array[i] | operand;
      }
      break;
    case 'xor':
      if (operand === undefined) throw new Error('Operand required for bitwise XOR');
      for (let i = 0; i < array.length; i++) {
        result[i] = array[i] ^ operand;
      }
      break;
    case 'not':
      ...
      break;
    case 'shiftLeft':
      if (operand === undefined) throw new Error('Operand required for left shift');
      for (let i = 0; i < array.length; i++) {
        result[i] = array[i] << operand;
      }
      break;
    case 'shiftRight':
      if (operand === undefined) throw new Error('Operand required for right shift');
      for (let i = 0; i < array.length; i++) {
        result[i] = array[i] >> operand;
      }
      break;
 ...
 */
export function extractChannel(imageData: ImageData, channel: number): Uint8Array {
 ...
 */
export function primeSequence(limit: number): number[] {
